...
const Form = () => {
  ...
  const fetchData = async () => {
    try {
      const result: SampleUser[] = await fetchApiRequest<SampleUser[]>(`${BASE_URL}/api/users/find-all`, createRequestInit('GET'));
      setUsers(result);
    } catch (error) {
      handleError(error);
    }
  ...
  const saveUser = async (first: string, last: string) => {
    ...
    try {
      ...
      await fetchApiRequest(`${BASE_URL}/api/users`, createRequestInit('POST', body));
      resetForm();
      fetchData();
      return true;
    } catch (error) {
  ...
  const deleteUser = async (first: string, last: string) => {
    try {
      await fetchApiRequest(`${BASE_URL}/api/users/${first}/${last}`, createRequestInit('DELETE'));
      fetchData();
      return true;
    } catch (error) {
      handleError(error);
